package com.gempukku.libgdx.util;
...
public enum Alignment {
    ...
    public float applyX(float width, float availableWidth) {
        return Align.isLeft(align) ? 0 : (Align.isRight(align) ? (availableWidth - width) : ((availableWidth - width) / 2));
    }

    public float applyY(float height, float availableHeight) {
        return Align.isTop(align) ? 0 : (Align.isBottom(align) ? (availableHeight - height) : ((availableHeight - height) / 2));
    }

    public Vector2 apply(float width, float height, float availableWidth, float availableHeight) {
        return apply(temp, width, height, availableWidth, availableHeight);
    }

    public Vector2 apply(Vector2 result, float width, float height, float availableWidth, float availableHeight) {
        return result.set(applyX(width, availableWidth), applyY(height, availableHeight));
    }
